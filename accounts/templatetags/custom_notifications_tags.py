...
def live_notify_badge(context, badge_class='live_notify_badge', extra_text=None):
    user = user_context(context)
    if not user:
        return ''

    if extra_text:
        html = "<span class='{badge_class}'>{unread} {extra_text}</span>".format(
...
def live_notify_list(list_class='live_notify_list'):
    html = "<li class='{list_class}'></li>".format(list_class=list_class)
    return format_html(html)
